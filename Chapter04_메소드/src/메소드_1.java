/*
	데이터 저장 ========> 데이터 가공(처리) ========> 가공된 데이터 출력
	 ...
	 1) 사용 목적 => 1.가독성 2.재사용성 3.속도 (최적화)
	 			   ---------------
	 			      ㄴ 메소드
	    = 분석 (가독성이 좋다) => 수정 (유지보수), 오류처리
	      기능별 분리 (로그인처리, 회원가입, 목록, 페이징...)
	    = 다른 클래스와 연결 (메세지)
	      객체지향 프로그램은 여러 개의 클래스를 만들어서 연관관계를 설정
	      => Scanner... (사용자 정의)
	         scan.nextInt();
	              ------- 입력된 정수 읽어오는 메소드 
	         ...
	         => 메소드는 한가지 기능만 수행이 가능하게 만든다
	            1) 입력
	            2) 요청 처리 => 기능에 따라 세분화 (여러개 메소드 제작이 가능)
	            3) 출력
	    = 반복 수행을 제거할 때 
	      오라클 연결
	      ---------
	  ...
	  3) 메소드 제작시 고민
		 ...
		 -------------------------------------
	  	 	  	X				O
	  	 	  	=> 메소드 자체에서 처리 => 출력을 메소드 안에서 
	  	 	  	void main(String[] args) => 리턴형이 없는 경우 => void 사용해야한다
	  	 	  	void println(String s)
         -------------------------------------
         		X				X
         		=> System.out,println() => 다음 줄에 출력 (거의 빈도수가 없다)
		 -------------------------------------

	  4) 메소드 선언 및 호출 방법 (클래스 블럭 안에서)
	  	 public class A
	  	 {
	  	 	-------------------------
	  	 	  메소드 선언 위치 
	  	 ...
	  	 형식)
	  	 	...
	  	 	-----
	  	 	{
	  	 		사용자 요청 처리 
	  	 		return 값;
	  	 		       -- 리턴형과 return 값이 일치 (리턴형이 실제 전송값보다 클 수도 있다)
	  	 	}             ㄴ 일치 시켜주는 게 가장 좋지만 리턴형이 더 커도 된다
	  	 	int 메소드명(매개변수)
	  	 	{
...
public class 메소드_1 {
	...
	static void a()
	{
		...
		return; // 생략이 가능 => 값을 보내줄 때 한개만 사용이 가능 
	}
