...
/*
 *  일반 데이터형 => 사용자 정의 데이터형 => 제공 안 됨 => 직접 제작
 *  class
 *  	- 데이터만 저장
 *  	- 메소드 저장
 *  
...
class MusicData
{
	...
	// 2. 생성자
	{
		// 예외 처리 => 에러르 미리 방지 => check, uncheck
		try
		{
			FileReader fr= new FileReader("c:\\javaDev\\genie.txt");
			String data="";
			int i=0; // 1글자씩 정수형으로 읽어옴 => char 변환
			
			while((i=fr.read())!=-1) // -1은 파일의 끝 => EOF(End Of File)
			{
				data+=(char)i;
			}
			fr.close(); // 파일 닫기(반드시 입력해야 함)
