...
public class 문자열_4 {
	...
	public static void main(String[] args) {
		...
		String[] title={
			...
			// 1. 순위가 가장 많이 상승한 노래 제목과 가수명을 출력하시오.
			int max=0;
			for(int i=0; i<title.length; i++) {
			...
			// 2. 노래명 중에 가장 긴 제목의 노래와 가수명, 앨범명을 출력하시오.
			int longTit=0;
			for(String t:title) {
				if(longTit<t.length()) {
					longTit=t.length();
			...
			System.out.println(longTit); // 가장 긴 제목 확인용
			int i=0;
			for(i=0; i<title.length; i++) {
				if(longTit==title[i].length()) {
					break;
				}
			}
			System.out.println("가장 긴 노래명:"+title[i]);
			System.out.println("가장 긴 노래 가수명:"+singer[i]);
			System.out.println("가장 긴 노래 앨범명:"+album[i]);
			// 배열 => length / 문자열 => length()
			
			System.out.println("========================================");
			System.out.println();
			
			// 3. 노래 중에 순위 상승한 노래명만 출력하시오.
			for(i=0; i<title.length; i++) {
				if(state[i].equals("상승"))
				{
			...
			//6. 검색어를 입력 받아서 검색어가 포함되어 있는 노래명을 출력하시오.
			System.out.println("검색어 입력:");
			String word=scan.next();
			
			// 방법1
			for(i=0; i<title.length; i++) {
				if(title[i].contains(word)) {
					System.out.println(title[i]);
				}
			}
